...
> 最后更新：2026-07-10  
> 状态：可开工（数据层 `ActionTimeline` / `ActionNotify` 已落地）  
...
2. **手动添加轨道**，在轨道内添加**类型化窗口**，右侧编辑细节，轨道内拖动位置与长短
3. VFX / SFX 窗口以**固定自然时长**为基准，拖动窗口长度按**倍率缩放播放速度**
4. 按逻辑帧 Scrub / 播放，预览动画 Pose、Hitbox、VFX/SFX
...
- [ ] 可手动添加轨道，并在轨内添加指定类型的窗口
- [ ] 选中窗口后右侧可编辑全部细节；轨道内可拖动位置与起止帧
- [ ] VFX/SFX 窗口拖长/拖短会按倍率缩放播放速度，自然时长作为 1.0× 基准
...
| VFX 预览语义 | 当前为选中常驻；目标为**时长窗口 + playbackSpeed** |
...
采用 **区间保活 + 倍率 Simulate**（参考 ARPG 的窗口内 Simulate，叠加本项目的拖长度=播放倍率）。不再把 VFX/SFX 当单帧点事件。
...
| 预览特效 | VFX/SFX 为**时长窗口**；拖长度改 `playbackSpeed`；预览用 `Simulate(t * speed)` |
| 预览调度 | `ActionEditorFramePlayer` 按逻辑帧驱动扩展；窗口 Enter/Tick/Exit 对齐 Runner |
...
│         ├ VfxPreviewExtension（窗口倍率 Simulate）         │
...
- **时间轴条目**：统一为可拖拽的**窗口**（`ActionTimelineItem`：`startFrame` / `endFrame` / `priority` / `trackName`）
- **区间窗口**（`ActionNotifyState`）：Hitbox / Hurtbox / Cancel / Movement / Rotation / **PlayVfx** / **PlaySfx**
- **瞬时信号**（可选保留）：`ActionEvent` 等仍可用点事件；**VFX/SFX 不再作为点事件**，改为带时长的窗口
- **暂留独立字段**：`phases[]`、`transitions[]`（可显示为轨道，Phase 1–2 不强制迁入 Timeline）
- **禁止**：再引入 `hitboxes[]` / `vfxEvents[]` / 散字段位移 / 单例 `RotationWindow`

#### 4.2.1 VFX / SFX 窗口数据（相对现状的变更）

现状代码中 `PlayVfxNotify : ActionNotify` 是点事件（`startFrame == endFrame`）。按本交互规格，需升级为**区间窗口**：

| 字段 | 含义 |
|------|------|
| `startFrame` / `endFrame` | 窗口在时间轴上的占用区间（可拖） |
| `prefab` / `audioClip` | 资源引用 |
| `naturalDurationSeconds` | **自然时长**（从 Prefab 粒子/AudioClip 读取或缓存）；对应倍率 1.0 |
| `playbackSpeed`（可派生） | `naturalDurationSeconds / windowDurationSeconds`；窗口拉长则减速，缩短则加速 |
| 变换 | `localOffset` / `localEulerAngles` / `localScale` / `parentToAttachPoint` |

```
windowDurationSeconds = (endFrame - startFrame + 1) / sampleRate
playbackSpeed         = naturalDurationSeconds / max(windowDurationSeconds, ε)
```

- 创建窗口时：读取资源自然时长 → 换算默认帧长 → `endFrame = startFrame + naturalFrames - 1`
- 拖动左右边：只改 `startFrame`/`endFrame`；**不改**自然时长；预览/运行时用 `playbackSpeed` 缩放 Simulate / 播放
- 拖动整体平移：保持窗口长度不变，只平移起止帧

> 实现阶段需将 `PlayVfxNotify` 从 `ActionNotify` 迁为 `ActionNotifyState`（或新增 `PlayVfxNotifyState`），并新增 `PlaySfxNotifyState`。属破坏性数据语义变更，按项目规则直接切换，不保留点事件双轨。
...
| 拖右边缘 | 改 `endFrame` |
| VFX/SFX 拖边缘 | 同上改区间；**隐含修改 `playbackSpeed`**，不改自然时长 |
...
| VFX | `timeline` 中 VFX 窗口列表 | **可缩放时长区间** | 青 |
| SFX | `timeline` 中 SFX 窗口列表（新增） | **可缩放时长区间** | 品红 |
  ...
  → ActionEditorPreviewSession.Tick()
       ...
       → ActionEditorFramePlayer.Advance(previousFrame, currentFrame)
            ...
            → Extensions:
                 ...
                 ├ VFX：进入窗口 Instantiate；窗口内 Simulate(localTime * playbackSpeed)
                 └ SFX：进入窗口预览播放；速度随 playbackSpeed（编辑器可用 AudioUtil）
```
...
#### 4.6.3 VFX / SFX 窗口预览（倍率缩放）

| 项 | 规格 |
|----|------|
| 进入窗口 | `previous` 不在区间且 `current` 在区间 → 创建预览实例 |
| 窗口内 Tick | `localFrame = current - startFrame`；`localTime = localFrame / sampleRate`；`Simulate(localTime * playbackSpeed, true, true/false)` |
| 离开窗口 | Destroy / 隐藏实例 |
| 拖长窗口 | `playbackSpeed < 1` → 粒子变慢，铺满更长区间 |
| 拖短窗口 | `playbackSpeed > 1` → 粒子变快，在更短区间播完 |
| 选中编辑辅模式 | 可强制显示选中 VFX 并开 Handles（调位置），与触发预览可切换 |
| 自然时长来源 | ParticleSystem 主模块 duration（含 startLifetime）或 AudioClip.length；缓存到窗口字段避免每帧解析 |
| Play Mode | 不跑编辑器预览 |

运行时应对齐同一公式：`ActionVfxPlayer`（或后继）在窗口激活期间按 `playbackSpeed` 驱动，而不是仅在单帧触发一次。
...
| 数据改造 | `PlayVfxNotify` → 区间 State；新增 `PlaySfxNotifyState`；运行时 `ActionVfxPlayer` 改窗口消费 |
...
| `vfxEvents[]` / 点事件 `PlayVfxNotify` | `timeline` 中 **VFX 区间窗口**（含 `naturalDurationSeconds`，拖长改倍率） |
| （无） | `timeline` 中 **SFX 区间窗口**（新增） |
