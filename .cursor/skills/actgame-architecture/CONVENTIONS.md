...
- **动作时间轴数据**：帧相关配置以 `ActionDefinition.Timeline` 为唯一真源；点事件用 `ActionNotify`（自定义 Event），区间窗口用 `ActionNotifyState`（含 VFX/SFX）；VFX/SFX 窗口时长可拖，倍率 = 自然时长 / 窗口时长；禁止重新引入 `hitboxes[]`、`vfxEvents[]`、散字段位移或单例 `RotationWindow`
